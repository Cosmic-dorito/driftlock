...
_CONFIG_NOTES = {
    "refit_screen_top_n": "ADR-0037",
    "pose_evidence_beta": "ADR-0032",
    "proposal_level": "ADR-0035",
    "drift_max_shear_px": "ADR-0036",
    "median_filter": "ADR-0027",
    "candidate_refit": "ADR-0019",
    "refit_screen_steps": "ADR-0025",
}


def build_config_block() -> str:
    """The shipped configuration, read out of ``localize.py::build_config`` itself.

    Hand-copied into three documents, this drifted within hours: after ADR-0037 moved the screen cut
    to 40, STATE and localize.py agreed and HANDOFF still said 30. An external reviewer found it
    before we did. Working rule 7 is "derive, never duplicate", and a configuration is exactly the
    kind of thing that gets duplicated because it is short enough to feel safe to retype.
    """
    import argparse
    ...
    shipped = L.build_config(argparse.Namespace(config="driftlock"))
    ...
    for field in fields(shipped):
        ...
        if value == getattr(defaults, field.name):
            continue
        note = _CONFIG_NOTES.get(field.name)
        suffix = f"  # {note}" if note else ""
        lines.append(f"    {field.name}={value!r},{suffix}")

    return "\n".join([
        CONFIG_BEGIN,
        "",
        "*Generated by `scripts/make_results_doc.py` from `localize.py::build_config`."
        ...
        *lines,
        ...
        "```",
        "",
        "Every field not listed is at its `PipelineConfig` default, which reproduces the sponsor's"
        " baseline.",
        "",
        CONFIG_END,
    ])
...
def main() -> int:
    ...
    for rel in CONFIG_DOCS:
        ...
        if not path.exists():
            continue
        if CONFIG_BEGIN not in path.read_text(encoding="utf-8"):
            print(f"  ! {rel} has no config markers - add them once, then this stays current")
            continue
        _splice(path, CONFIG_BEGIN, CONFIG_END, config_block)
        print(f"  Regenerated the config block in {rel}")
