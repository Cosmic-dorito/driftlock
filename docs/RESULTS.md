...
**Deadline:** 16 Aug 2026 · Current numbers are generated into section 2 from `results/`.

---

## 0. Headline

> **The current headline lives in section 2 below, and is generated from `results/`.**
> It used to be duplicated here by hand and drifted: this section carried 27.5 / 33.3 / 33.3 %
> mis-lock long after the measured values were 25.0 / 23.3 / 16.7 %. One generated source, cited
> from everywhere else, is the only arrangement that does not rot.

Shipped configuration: pyramid pose measurement -> full-resolution correlation -> per-candidate pose
refit -> sub-pixel DFT -> two-axis drift cancellation. Eight direct dependencies, all pinned, no GPU, no learned weights.

**What is still unsolved:** selection. Every pass rate is capped by the mis-lock rate. Six
hand-designed re-ranking criteria have been tried and all six failed (ADR-0024); the only stage that
helped re-scores by the *same* criterion at a better geometry.
...
| Threads | see the environment line under the headline table — the current measurement is not single-threaded |
...
*Generated by `scripts/make_results_doc.py` from `results/` on 2026-08-18 at commit `edf44cb`. Do not edit by hand — regenerate.*

| Split | Config | Mis-lock | Median err (px) | pass@1px | pass@0.5px | Runtime p50 |
|---|---|---|---|---|---|---|
| **sponsor `verify`** (40) | baseline | 25.0% | 1.102 | 40.0% | 17.5% | 20 ms |
| *their generator, fixed 10:1, no rotation* | **DriftLock** | **0.0%** | **0.179** | **97.5%** | **92.5%** | 632 ms (32x base) |
| **bench** (30) | baseline | 76.7% | 326.905 | 10.0% | 3.3% | 19 ms |
| *ours: 9–11:1 magnification, ±2° rotation, DRAM* | **DriftLock** | **13.3%** | **0.300** | **83.3%** | **66.7%** | 568 ms (29x base) |
| **holdout FinFET** (30) | baseline | 90.0% | 359.893 | 3.3% | 3.3% | 20 ms |
| *held-out architecture, never tuned on* | **DriftLock** | **3.3%** | **0.214** | **90.0%** | **76.7%** | 564 ms (29x base) |

**Aggregate mis-lock across all 100 evaluated pairs: 5/100 = 5.0%.**
